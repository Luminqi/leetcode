...
import { ClearButtonProps, ToggleButtonProps, ItemProps } from './types'

const Warn = React.memo(({ visibility, ...props }: { visibility: boolean }) => {
  return (
    <div className="autocomplete-warn" style={{ visibility: visibility ? 'visible' : 'hidden' }} {...props}>
      <div className="autocomplete-warn-content">
        <span>You input too much charactors</span>
      </div>
      <div className="autocomplete-warn-arrow" />
    </div>
  )
})
...
const ToggleButton = React.memo(({ isOpen, onClick, ...props }: ToggleButtonProps) => {
  return (
    <button className="autocomplete-togglebtn" onClick={onClick} {...props}>
      <svg
  ...
  ({
    ...
    return showMenu ? (
      <ul className="autocomplete-menu">
        {items!.map((item) => (
          <li className="autocomplete-item" key={item} {...getItemProps({ selectedValue: item })}>
            {item}
          </li>
        ))}
      </ul>
...
export { Warn, SearchIcon, Loading, ClearButton, ToggleButton, Menu }
